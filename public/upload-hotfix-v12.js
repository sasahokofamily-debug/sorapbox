...
function parentFolder(){return ROOT}
async function uploadOne(file){
  ...
  try{
    ...
    node.status.textContent="100% ・ 完了";node.bar.style.width="100%";toast(`${file.name} を保存しました.`,`success`);setTimeout(()=>{node.root.remove();if(!queue.children.length)queue.classList.add("hidden")},1400);
...
console.info("sorapbox FREE upload hotfix v1.2.1 loaded");
